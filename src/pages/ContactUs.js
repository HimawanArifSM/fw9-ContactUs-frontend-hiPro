...
const contactUsSchema = Yup.object().shape({
    fullname: Yup.string().length({min: 4}).required('Required'),
    email: Yup.string().email("Invalid email format").required('Required'),
    messages: Yup.string().length({max: 255}).required('Required')
})
    ...
    return(
        <Form className="flex flex-col gap-10">
            <Form.Group className="border-b-[1px] border-black pb-2 outline-hidden">
                <Form.Control isInvalid={!!props.errors.email} onChange={props.handleChange} name="fullname" className="outline-none w-full" placeholder="Name" />
                <Form.Control.Feedback type="invalid">{props.errors.fullname}</Form.Control.Feedback>
            </Form.Group>
            <Form.Group className="border-b-[1px] border-black pb-2 outline-hidden">
                <Form.Control isInvalid={!!props.errors.email} onChange={props.handleChange} name="email" className="outline-none w-full"placeholder="E-mail" />
                <Form.Control.Feedback type="invalid">{props.errors.email}</Form.Control.Feedback>
            </Form.Group>
            <Form.Group className="border-b-[1px] border-black pb-2 outline-hidden">
            <textarea isInvalid={!!props.errors.email} onChange={props.handleChange} name="messages" className="w-full h-full outline-none" form="messages" placeholder="Messages" ></textarea>
        ...
        <div className="lp-background min-h-screen ">
            <div className="flex justify-evenly bg-black/75 min-h-screen p-5 items-center">
                ...
                <div className="bg-white p-16 flex flex-col gap-10">
                    ...
                    <Formik 
                    onSubmit={onSubmit}
                    initialValues={{email: '', password:''}} validationSchema={contactUsSchema}>
